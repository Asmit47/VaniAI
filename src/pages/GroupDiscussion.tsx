...
const GroupDiscussion = () => {
  ...
  const handleStartDiscussion = async () => {
    ...
    try {
      // Create local AI participants
      const pool = [
        { name: 'Aisha', personality: 'Analytical' },
        { name: 'Ravi', personality: 'Creative' },
        { name: 'Maya', personality: 'Pragmatic' },
        { name: 'Karan', personality: 'Skeptical' },
      ];
      const selected = pool.slice(0, config.participants);
  ...
  const handleAITurn = async (participant: {name: string; personality: string}, history: Message[]) => {
    ...
    try {
      ...
      const prompt = `You are ${participant.name}, an AI participant with a ${participant.personality} personality in a group discussion about "${config.topic}". Given the conversation so far:\n\n${historyText}\n\nRespond concisely (1-2 sentences), in a natural conversational tone.`;
      const response = await generateGeminiResponse(prompt);
      const aiMessage: Message = { role: participant.name, content: response, personality: participant.personality };
  ...
  const getFeedback = async (history: Message[]) => {
    ...
    try {
      ...
      const prompt = `You are a group discussion coach. Analyze the following conversation on "${config.topic}" and provide concise, actionable feedback (participation, clarity, listening, and 3 improvements). Return plain text.\n\n${conversationText}`;
